...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        if(pid == 0)
        {
            ...
            counter_malicious = process_directory(argv[i]);
    ...
    while ((pid = wait(&status)) > 0) {
        if (WIFEXITED(status)) {
            int malicious_count = WEXITSTATUS(status);
            printf("Child with pid = %d, found %d malicious files\n", pid, malicious_count);
        }
    }
